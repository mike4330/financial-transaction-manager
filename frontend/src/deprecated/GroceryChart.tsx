...
export const GroceryChart: React.FC = () => {
  ...
  useEffect(() => {
    const fetchGroceryData = async () => {
      try {
        ...
        // Fetch filtered grocery transactions
        ...
        setError(err instanceof Error ? err.message : 'Failed to load grocery data');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) {
    return (
      <div className="card">
        <h3 className="text-xl mb-4">🛒 Grocery Spending Over Time</h3>
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h3 className="text-xl">🛒 Grocery Spending Over Time</h3>
      ...
      {data.length > 0 && (
        <div style={{ marginTop: '1rem', fontSize: '0.875rem', color: '#666' }}>
          <p>Showing grocery spending from {data[0]?.month} to {data[data.length - 1]?.month}</p>
        </div>
      )}
    </div>
  );
