...
### Converting Investment Transactions to Income (Stock Sales)
When stock sales need to be reclassified as income rather than investment transactions:

```bash
# 1. Find the transaction (example: transaction ID 10549)
sqlite3 transactions.db "SELECT t.id, t.run_date, t.account, t.action, t.amount, c.name as category, s.name as subcategory FROM transactions t LEFT JOIN categories c ON t.category_id = c.id LEFT JOIN subcategories s ON t.subcategory_id = s.id WHERE t.id = 10549;"

# 2. Create Income/Stock Sale category and subcategory
python3 -c "
from database import TransactionDB
db = TransactionDB()
income_cat_id = db.get_or_create_category('Income')
stock_sale_subcat_id = db.get_or_create_subcategory('Stock Sale', income_cat_id)
print(f'Income category ID: {income_cat_id}, Stock Sale subcategory ID: {stock_sale_subcat_id}')
"

# 3. Update transaction to Income/Stock Sale with positive amount
sqlite3 transactions.db "UPDATE transactions SET category_id = 205, subcategory_id = [STOCK_SALE_ID], amount = ABS(amount) WHERE id = 10549;"

# 4. Add Stock Sale to budget if not already present
curl -X POST "http://localhost:5000/api/budget/2025/9/add-category" -H "Content-Type: application/json" -d '{
  "category": "Income",
...
# 5. If transaction doesn't appear in budget, check for subcategory ID mismatches
sqlite3 transactions.db "SELECT id, name FROM subcategories WHERE name = 'Stock Sale';"
# If duplicates exist, update transaction to match budget item:
sqlite3 transactions.db "UPDATE transactions SET subcategory_id = [BUDGET_SUBCATEGORY_ID] WHERE id = 10549;"

# 6. Verify transaction appears in both transaction list and budget
curl -s "http://localhost:5000/api/transactions?category=Income"
curl -s "http://localhost:5000/api/budget/2025/9"
```

**Common Issues:**
- **Duplicate subcategories**: Check for multiple "Stock Sale" subcategories with different IDs
- **ID mismatches**: Transaction subcategory_id must match budget item subcategory_id
- **Amount sign**: Ensure income transactions have positive amounts
- **Budget line item missing**: Income categories need budget line items to display actual amounts
